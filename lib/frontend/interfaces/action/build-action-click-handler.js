...
const buildActionClickHandler = options => {
  ...
  const handleActionClick = event => {
    ...
    if ((0, _actionHasComponent.actionHasComponent)(action)) {
      ...
      var _location$search, _location;

      const url = new URL(`relative:${href}`);
      const hrefParams = new URLSearchParams(url.search);
      const currentParams = new URLSearchParams(action.showInDrawer ? (_location$search = (_location = location) === null || _location === void 0 ? void 0 : _location.search) !== null && _location$search !== void 0 ? _location$search : '' : '');
      Object.entries(Object.fromEntries(currentParams.entries())).forEach(([key, value]) => {
        hrefParams.append(key, value);
      });
      push(`${url.pathname}?${hrefParams.toString()}`, {
